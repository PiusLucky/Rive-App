...
const MainButton = forwardRef<HTMLButtonElement, MainButtonProps>(
  (
    ...
    const propWidth =
      width === "full_width" ? "w-full" : width ? width : "w-[144px]";
    ...
    return !isLoading ? (
      <Button
        ...
        } text-white shadow-xl ${propWidth} md:${propWidth}  select-none rounded-[40px] hover:opacity-90 ${variant_hover} ${size_height} ${classes}`}
      ...
      <Button
        className={`bg-primary text-white ${propWidth} md:${propWidth} select-none rounded-[40px] cursor-not-allowed ${size_height} ${
          classes ? classes : ""
        }`}
        ref={ref}
        disabled
